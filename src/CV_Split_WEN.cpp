...
double CV_Split_WEN::Logistic_Deviance(arma::mat x, arma::vec y,
  ...
  //return(arma::mean((std::exp(1)/(std::exp(1)-1))*(-2*arma::log((1+(2*y-1))*arma::mean(y)/4+(1-(2*y-1))*(1-arma::mean(y))/4)%arma::pow((2*(2*y-1)%arma::pow((2*y-1)+2*(arma::mean(intercept) + x * arma::mean(betas, 1))-1,-1)),1/(1-arma::mean(y))%(arma::exp((-2*y+1)%(arma::mean(intercept) + x * arma::mean(betas, 1)))-std::exp(-1)))));
         
  return(arma::mean(
    (std::exp(1)/(std::exp(1)-1))*
         (-2*arma::log(
           (1+(2*y-1))*arma::mean(y)/4+(1-(2*y-1))*(1-arma::mean(y))/4)%
          (2*(2*y-1)%
           arma::pow(
             (2*y-1)+2*(arma::mean(intercept) + x * arma::mean(betas, 1))-1,-1)),1/(1-arma::mean(y)%
          (
            arma::exp(
              (-2*y+1)%(arma::mean(intercept) + x * arma::mean(betas, 1))
            )-std::exp(-1)))));
}
// Gamma Deviance (MSPE)
