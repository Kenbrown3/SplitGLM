...
double CV_Split_WEN::Logistic_Deviance(arma::mat x, arma::vec y,
  ...
  return((std::exp(1)/(std::exp(1)-1))*(-2*arma::log((1+(2*y-1))*arma::accu(y)/(4*x.n_rows)+(1-(2*y-1))*arma::accu(y-1)/(4*x.n_rows))*(2*(2*y-1)%arma::pow((2*y-1)+2*(arma::mean(intercept) + x * arma::mean(betas, 1))-1,-1)%(arma::exp((2*y-1)+2*(arma::mean(intercept) + x * arma::mean(betas, 1))-1))-std::exp(-1))))
}
// Gamma Deviance (MSPE)
