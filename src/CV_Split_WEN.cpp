...
double CV_Split_WEN::Logistic_Deviance(arma::mat x, arma::vec y,
  ...
  //exp loss
  return(arma::accu(arma::exp((2*y-1)%((arma::mean(intercept) + x * arma::mean(betas, 1)-1)*2)))/x.n_rows);
  //return(arma::accu(arma::exp(-1*((2*y-1)%((arma::mean(intercept) + x * arma::mean(betas, 1))/(1+arma::mean(intercept) + x * arma::mean(betas, 1))))))/x.n_rows);
